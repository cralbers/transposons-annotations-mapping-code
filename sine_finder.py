...
def prompter(oname, **kwargs):
	...
	while 1:
		...
		print
		try:
			choice = raw_input("%s%s: " % (oname, app))
...
class MotifFinder:
	...
	def _revcomp(self, seq):
		...
		table = string.maketrans(*complstr)
		seq = string.translate(seq, table)
	...
	def set_pattern(self, p=[]):
		...
		if p:
			...
			self.groups = map(lambda v: v[0], p)
		else:
			self.p = None
			self.groups = None
...
class SINEFinder(MotifFinder):
	...
	def __len__(self):
		...
		if self.status & 2:
			return len(filter(lambda d: d['TSD_status'] == True, self.mm))
		else:
			return 0
...
def run(seqfile, **kwargs):
	...
	if kwargs['verbose'] == 1:
		cfgi = options.items()
		cfgi.sort()
		cfgt = "\n".join(map(lambda i: "   % -30s = %s" % i, cfgi))
		...
		if len(sys.argv) == 1:
			...
			while 1:
				...
				while 1:
					...
					ff = filter(lambda f: os.path.isfile(os.path.join('.', f)) and not f.startswith('.') and filter_extension(f), os.listdir('.'))
					dd = filter(lambda f: os.path.isdir(os.path.join('.', f)), os.listdir('.'))
					ddi = map(lambda d: "CHANGE DIR TO..." + d, dd)
					files = [('..', 'UP ONE DIR')] + zip(dd, ddi) + zip(ff, ff)
				...
				for arg in (
					...
					print (argument % arg)
				print
				choice = raw_input("Start job with these arguments: y(es), n(o) or q(uit)? ")
				if choice in 'yY':
					break
				elif choice in 'nN':
					continue
